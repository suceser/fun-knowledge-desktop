...
export function useChatState(): ChatStateHook {
  ...
  const currentAssistant = assistants.find(
    (a) => a.id === chatState.currentAssistantId,
  );
  ...
  const initializeData = useCallback(async () => {
    try {
      ...
      if (lastTopicId && loadedTopics.find((t) => t.id === lastTopicId)) {
        ...
        setChatState((prev) => ({
          ...prev,
          currentTopicId: loadedTopics[0].id,
        }));
      }
      ...
      if (success) {
        ...
        if (chatState.currentTopicId === topicId) {
          ...
          if (remainingTopics.length > 0) {
            setChatState((prev) => ({
              ...prev,
              currentTopicId: remainingTopics[0].id,
            }));
          } else {
  ...
  const renameTopic = useCallback(
    async (topicId: string, newTitle: string): Promise<boolean> => {
      const updatedTopic = storageService.updateTopic(topicId, {
        title: newTitle,
      });
    ...
    async (topicId: string): Promise<boolean> => {
      ...
      if (success) {
        setTopics((prev) =>
          prev.map((t) =>
            t.id === topicId
              ? { ...t, messages: [], messageCount: 0, lastMessage: undefined }
              : t,
          ),
        );
      }
  ...
  const sendMessage = useCallback(
    async (content: string, attachments?: any[]) => {
      ...
      // 添加到话题
      const updatedTopic = storageService.addMessage(
        currentTopic.id,
        userMessage,
      );
      if (updatedTopic) {
        setTopics((prev) =>
          prev.map((t) => (t.id === currentTopic.id ? updatedTopic : t)),
        );
      }
      ...
      try {
        ...
        // 获取上下文消息
        const contextMessages = storageService.getContextMessages(
          currentTopic.id,
        ...
        // 添加到话题
        const topicWithAssistantMsg = storageService.addMessage(
          currentTopic.id,
          assistantMessage,
        );
        if (topicWithAssistantMsg) {
          setTopics((prev) =>
            prev.map((t) =>
              t.id === currentTopic.id ? topicWithAssistantMsg : t,
            ),
          );
          setChatState((prev) => ({
            ...prev,
        ...
        setTopics((prev) =>
          prev.map((t) =>
            t.id === currentTopic.id
              ? {
                  ...t,
                  messages: t.messages.filter((m) => m.id !== userMessage.id),
                }
              : t,
          ),
        );
  ...
  const editMessage = useCallback(
    async (messageId: string, newContent: string): Promise<boolean> => {
      ...
      const updatedMessage = storageService.updateMessage(
        currentTopic.id,
        messageId,
        {
          content: newContent,
  ...
  const switchAssistant = useCallback(
    (assistantId: string) => {
      ...
      if (settings) {
        ...
        setSettings((prev) =>
          prev ? { ...prev, defaultAssistantId: assistantId } : null,
        );
      }
    },
    [settings],
  ...
  const updateAssistant = useCallback(
    async (
      ...
      const updatedAssistant = storageService.updateAssistant(
        assistantId,
        updates,
      );
      ...
      if (updatedAssistant) {
        setAssistants((prev) =>
          prev.map((a) => (a.id === assistantId ? updatedAssistant : a)),
        );
        return true;
      }
